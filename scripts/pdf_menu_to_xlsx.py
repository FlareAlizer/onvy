...
_СОСТАВ_МАРКЕРЫ = ("украшается", "подается", "подаётся", "заправка", "маринад", "соус")
...
def _вес_состава(text: str) -> int:
    ...
    if _посуда(text):
        return -1
    низ = text.casefold()
    return len(_НУМЕРАЦИЯ.findall(text)) * 3 + sum(m in низ for m in _СОСТАВ_МАРКЕРЫ)
