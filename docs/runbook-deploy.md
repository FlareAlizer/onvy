...
Ежедневный дамп с ротацией — `scripts/backup-postgres.sh`. Поставить в cron:
...
`BACKUP_DIR`, `RETENTION_DAYS`). Проверить, что бэкап реально пишется:
...
**Непроверенный бэкап — не бэкап.** Тестируй восстановление на пустой БД/тестовом
сервере хотя бы раз до пилота, не только в день инцидента.
