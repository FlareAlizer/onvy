 ...
 */
// Чужие контексты, которые надо будить тем же касанием. Микрофон постоянного
// прослушивания живёт в собственном AudioContext, и он тоже стартует спящим:
// браузер поднимает звук только по жесту человека. Спящий контекст не вызывает
// onaudioprocess вовсе — микрофон открыт, индикатор записи горит, а обработчик
// не получает ни одного буфера. Наружу это выглядит как «Онви не реагирует на
// голос», притом что кнопка отвечает: она поднимает свой контекст прямо внутри
// нажатия, то есть в момент, когда жест ещё действует.
const чужие = new Set<AudioContext>();

/** Будить этот контекст вместе с общим выходом. Возвращает отписку. */
export function будитьВместеСВыходом(c: AudioContext): () => void {
  чужие.add(c);
  return () => чужие.delete(c);
}

function разбудить(): void {
  for (const чужой of чужие) {
    if (чужой.state === 'suspended') void чужой.resume().catch(() => {});
  }
  const c = контекст();
  if (!c) return;
  void c.resume().catch(() => {});
