...
async def выполнить(*, require_wake_word: bool = False, **переопределения):
...
class TestРазговорСоСтоломНеПокидаетСервер:
    """Микрофон в режиме «Онви слушает» открыт всю смену.

    Всё, что официант говорит гостям за столами, попадает в конвейер, но нашим
    разговором не является: гость согласия не давал. Такая фраза обязана
    исчезнуть сразу после разбора — не сохраниться, не уехать обратно на телефон
    и не попасть на экран управляющего.

    Проверяем не «где-то не сохраняется», а что распознанного текста нет ни в
    одном поле результата: сохранять и показывать нечего, если нечего брать.
    """

    async def test_текст_чужого_разговора_не_попадает_в_результат(self):
        итог = await выполнить(
            распознано="слушайте я вам принесу счёт через минуту",
            require_wake_word=True,
        )

        assert итог.kind is IntentKind.IGNORED
        # Ни одно поле не должно нести сказанного гостю.
        assert итог.query_text == ""
        assert итог.answer_text == ""
        assert итог.payload == ""
        assert итог.grounded_on == ()
        assert итог.audio is None

    async def test_ассистент_не_вызывается_и_не_озвучивает(self):
        """Чужая фраза не должна стоить ни запроса к модели, ни синтеза —
        иначе смена в зале выжигает квоту на разговорах с гостями."""
        итог = await выполнить(
            распознано="а вы у нас первый раз",
            require_wake_word=True,
        )
    ...
    async def test_обращение_к_ассистенту_обрабатывается_как_обычно(self):
        """Обратная сторона: режим прослушивания не должен оглушить продукт."""
        итог = await выполнить(
            распознано="Онви что в составе лагмана",
            require_wake_word=True,
        )
...
class TestОтказРаспознаванияПриОткрытомМикрофоне:
    """Найдено ревью: отказ облака проверялся ДО правила про обращение.

    В режиме «Онви слушает» микрофон открыт всю смену. Если распознавание
    отвалилось, мы не знаем, была ли фраза обращением к нам, — а значит с
    большой вероятностью это разговор со столом. Телефон обязан промолчать:
    иначе при перебоях в облаке он вслух говорит «Не расслышал» поверх
    разговора с гостем, и тем чаще, чем хуже связь.
    """

    async def test_молчит_вместо_ответа_поверх_разговора_с_гостем(self):
        итог = await выполнить(ошибка_asr="таймаут", require_wake_word=True)
    ...
    async def test_по_кнопке_отказ_по_прежнему_объясняется_голосом(self):
        """Кнопку человек нажал сам и ждёт ответа — молчание тут было бы хуже."""
        итог = await выполнить(ошибка_asr="таймаут")
