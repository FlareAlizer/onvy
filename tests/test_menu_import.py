...
class TestЦенаКакЕёПишутВВыгрузках:
    """Цена — обязательное поле: не разобрали, теряется всё блюдо, а не ячейка.

    Выгрузки из учётных систем регулярно приходят с разделителем тысяч и валютой
    прямо в ячейке. Отказываться от такого файла значит отказаться от половины
    меню заведения — а прислать его должен человек, который не разбирается в
    технике и переделывать ничего не будет.
    """
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("520", Decimal("520")),
            ("520,50", Decimal("520.50")),
            ("1 200", Decimal("1200")),
            ("1\u00a0200", Decimal("1200")),  # неразрывный пробел из Excel
            ("520 ₽", Decimal("520")),
            ("1 200 руб", Decimal("1200")),
            ("520 рублей", Decimal("520")),
        ],
    )
    def test_цена_разбирается(self, ячейка, ожидание):
        строки = parse_menu_file(
            f"Название;Цена\nПлов;{ячейка}\n".encode(), filename="m.csv"
        )

        assert строки[0].price == ожидание, f"«{ячейка}» не разобралась"
        assert строки[0].is_valid

    def test_слова_вместо_цены_остаются_ошибкой(self):
        """Терпимость не должна превращаться в угадывание: «не указана» — это
        не цена, и управляющий должен увидеть это в предпросмотре."""
        строки = parse_menu_file(
            "Название;Цена\nПлов;не указана\n".encode(), filename="m.csv"
        )

        assert not строки[0].is_valid
