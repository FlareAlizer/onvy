...
async def push_to_talk(
    ...
    # Постоянное прослушивание оставляет след ровно в одном месте — здесь.
    #
    # Отброшенная фраза никуда не записывается (и не должна), поэтому по базе
    # «телефон молчит» и «телефон шлёт, а мы отбрасываем» выглядят одинаково —
    # никак. Без этой строки диагностировать неработающий голос нечем: кнопка
    # при этом отвечает, потому что ей обращение не нужно.
    #
    # В лог идут только вид итога и исковерканное написание имени ассистента.
    # Речи гостя здесь нет и быть не может (см. почти_обращение).
    if always_on:
        logger.info(
            "Постоянное прослушивание: сотрудник=%s итог=%s байт=%s%s",
            current.id,
            outcome.kind.value,
            len(audio_bytes),
            f" услышано вместо имени: {outcome.wake_near_miss!r}"
            if outcome.wake_near_miss
            else "",
        )
