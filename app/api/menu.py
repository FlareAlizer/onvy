...
async def import_menu(
    ...
    """Импорт CSV или Excel (.xlsx). По умолчанию dry-run: управляющий сначала
    видит, что создастся и что обновится, и только повторным вызовом с
    dry_run=false применяет.

    Формат определяется по содержимому файла, не по расширению: заведение
    присылает файл как есть, мы не выбираем формат заранее. Нужные колонки —
    «название» и «цена» обязательны, остальные («категория», «состав»,
    «аллергены», «вес», «время отдачи», «острота») — опциональны; порядок в
    файле не важен и формулировка заголовка может отличаться (см. алиасы в
    app/services/menu_import.py) — ищем по смыслу, а не по точному тексту.
    Шапка не обязана быть первой строкой: просматриваем первые 15 строк файла
    и берём ту, где узнано больше всего колонок (в реальных файлах сверху
    бывает название заведения, дата, пустые строки).

    Кодировка CSV — UTF-8 или CP1251, разделитель «,» или «;» — определяются
    автоматически (реальный случай: выгрузка из Excel под русской Windows
    приходит в CP1251 с «;»). Пустая ячейка техкарты значит «данных нет»; для
    аллергенов явное слово «нет» — это «аллергенов нет» (проверено), а не то
    же самое, что пусто.
    """
    require_own_venue(current, venue_id)
    raw = await file.read()
